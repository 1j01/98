...
function FolderView(folder_path, { asDesktop = false } = {}) {
	...
	function navigate_grid(move_x, move_y) {
		...
		const rect = $starting_icon[0].getBoundingClientRect();
		// This code was quickly generated by AI.
		// Really it should use the grid_size_x/y, and it could
		// skip the getBoundingClientRect in cases where it's not scrolled offscreen by using document.elementFromPoint perhaps,
		// but I don't know if that would be faster. It could use a data structure though for the layout, aside from the DOM.
		let x = rect.left + rect.width / 2 + move_x * rect.width;
		let y = rect.top + rect.height / 2 + move_y * rect.height;
		const $icon = $folder_view.find(".desktop-icon").filter(function () {
